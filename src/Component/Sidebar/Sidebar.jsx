...
import {
  Users, Building2, Hospital, UserCog, Calendar, Umbrella,
  CheckCircle, Clock, FileText, LayoutDashboard, ChevronLeft,
  ChevronRight, ChevronDown, UserCircle, ClipboardList, BarChart,
  Bell, Mail, Users2, BellRing, Inbox,
} from 'lucide-react';
  ...
  const menuItems = [
    ...
    },
    {
      ...
      subItems: [
        ...
        { icon: Building2, label: 'Practices', path: '/practices' },
      ],
    },
    {
      icon: UserCog,
  ...
  useEffect(() => {
    ...
    menuItems.forEach((item) => {
      if (item.hasDropdown) {
        ...
        if (hasActive && !isCollapsed) newExpanded[item.id] = true;
      }
    });
  ...
  return (
    <>
      <style>{`
        ...
        .hover-popup.active { opacity: 1; visibility: visible; transform: translateX(0) scale(1); pointer-events: auto; }
        .chevron-rotate { transition: transform 0.2s cubic-bezier(0.4,0,0.2,1); }
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1 sidebar-scroll">
          {menuItems.map((item) => {
            ...
            return (
              <div key={item.id} className="relative" onMouseEnter={() => handleMouseEnter(item)} onMouseLeave={handleMouseLeave}>
                <button
                  onClick={() => item.hasDropdown ? toggleDropdown(item.id) : handleNavigate(item.path)}
                  className={`w-full flex items-center gap-3 px-3 py-3.5 rounded-lg transition-all duration-200 ease-in-out group relative ${
                    isActive ? 'bg-core-primary-50 text-core-primary-500' : 'text-secondary hover:bg-primary hover:text-core-primary-500'
                ...
                >
                  ...
                  <Icon
                    size={18}
                    className={`transition-all duration-200 ${isActive ? 'text-core-primary-500' : 'text-muted group-hover:text-core-primary-500'} ${isCollapsed && !isHovered ? '' : 'ml-1'}`}
                  ...
                  {(!isCollapsed || isHovered) && (
                    <>
                      ...
                      {item.hasDropdown && (
                        <ChevronDown
                          size={16}
                          className={`chevron-rotate ${isDropdownOpen ? 'rotated' : ''} transition-all duration-200 ${isActive ? 'text-core-primary-500' : 'text-muted'}`}
                        />
                      )}
                    </>
                  )}
                    ...
                    <div className="ml-4 space-y-1 border-l-2 border-core-primary-100 pl-1">
                      {item.subItems.map((subItem) => {
                        ...
                        const isSubActive =
                          ...
                          (subItem.path !== '/email-activity' && location.pathname.startsWith(subItem.path + '/'));
                        return (
                          <button
                            key={subItem.path}
                            onClick={() => handleNavigate(subItem.path)}
                            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 ease-in-out group ${
                              isSubActive ? 'bg-core-primary-50 text-core-primary-500' : 'text-secondary hover:bg-primary hover:text-core-primary-500'
                            }`}
                          >
                            <SubIcon size={16} className={`transition-all duration-200 ${isSubActive ? 'text-core-primary-500' : 'text-muted group-hover:text-core-primary-500'}`} strokeWidth={isSubActive ? 2.5 : 2} />
                    ...
                    <div className="py-2">
                      {item.subItems.map((subItem) => {
                        ...
                        return (
                          <button
                            key={subItem.path}
                            onClick={() => { handleNavigate(subItem.path); setHoveredItem(null); }}
                            className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm font-normal transition-all duration-200 ${
                              isSubActive ? 'bg-core-primary-50 text-core-primary-500 font-medium' : 'text-secondary hover:bg-primary hover:text-core-primary-500'
                            }`}
                          >
                            <SubIcon size={16} className={`shrink-0 ${isSubActive ? 'text-core-primary-500' : 'text-muted'}`} strokeWidth={isSubActive ? 2.5 : 2} />
        ...
        <div className={`transition-all duration-300 ${isCollapsed && !isHovered ? 'p-3' : 'p-4'}`}>
          <div className={`flex items-center gap-3 ${isCollapsed && !isHovered ? 'justify-center' : ''}`}>
            <div className="w-9 h-9 bg-gradient-to-br from-core-primary-500 to-core-primary-600 rounded-full flex items-center justify-center text-white font-semibold text-sm shrink-0 shadow-md">
              AS
            </div>
            {(!isCollapsed || isHovered) && (
              <>
