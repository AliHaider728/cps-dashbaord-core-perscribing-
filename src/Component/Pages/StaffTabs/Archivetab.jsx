...
const ArchiveTab = ({ staffData }) => {
  ...
  const handleRestore = (id) => {
    ...
    if (window.confirm(`Are you sure you want to restore "${item.itemName}"?`)) {
      ...
      // In a real app, this would move the item back to its original location
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-primary">Archived Items</h2>
          <p className="text-secondary text-sm mt-1">View and manage archived staff records</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-6 gap-3">
        <div className="bg-primary rounded-xl border border-border p-4 hover:shadow-md transition-shadow">
          ...
          <div className="text-sm text-secondary mt-1">Total Items</div>
        </div>
        <div className="bg-blue-50 rounded-xl border border-blue-200 p-4 hover:shadow-md transition-shadow">
          ...
          <div className="text-sm text-blue-700 mt-1">Documents</div>
        </div>
        <div className="bg-purple-50 rounded-xl border border-purple-200 p-4 hover:shadow-md transition-shadow">
          ...
          <div className="text-sm text-purple-700 mt-1">Training</div>
        </div>
        <div className="bg-yellow-50 rounded-xl border border-yellow-200 p-4 hover:shadow-md transition-shadow">
          <div className="text-2xl font-bold text-yellow-600">{stats.notes}</div>
          <div className="text-sm text-yellow-700 mt-1">Notes</div>
        </div>
        <div className="bg-green-50 rounded-xl border border-green-200 p-4 hover:shadow-md transition-shadow">
          <div className="text-2xl font-bold text-green-600">{stats.invoices}</div>
          <div className="text-sm text-green-700 mt-1">Invoices</div>
        </div>
        <div className="bg-orange-50 rounded-xl border border-orange-200 p-4 hover:shadow-md transition-shadow">
          <div className="text-2xl font-bold text-orange-600">{stats.restorable}</div>
          <div className="text-sm text-orange-700 mt-1">Restorable</div>
        </div>
      </div>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start gap-3">
        ...
        <div>
          <h4 className="font-medium text-yellow-800 mb-1">Archived Items Information</h4>
          <p className="text-sm text-yellow-700">
            Items are archived when they are replaced, expired, or no longer needed. Some items can be restored, 
            while others have been permanently superseded. Archived items are retained for record-keeping purposes.
          </p>
        </div>
      </div>
      ...
      {filteredItems.length > 0 ? (
        ...
        <div className="bg-primary rounded-xl border border-border p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-primary mb-2">No Archived Items</h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-secondary rounded-xl shadow-xl max-w-2xl w-full">
            <div className="bg-gray-100 border-b border-border px-6 py-4 flex items-center justify-between">
              <h3 className="text-xl font-bold text-primary">Archived Item Details</h3>
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-4 pb-4 border-b border-border">
                ...
                <div className="flex-1">
                  <h3 className="text-xl font-bold text-primary mb-1">{viewingItem.itemName}</h3>
                  <div className="flex gap-2">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${getItemTypeColor(viewingItem.itemType)}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="font-medium text-primary">{viewingItem.originalDate}</div>
                </div>
                <div>
                  <div className="text-xs text-muted mb-1">Archived Date</div>
                  <div className="font-medium text-primary">{viewingItem.archivedDate}</div>
                </div>
                <div>
                  <div className="text-xs text-muted mb-1">Archived By</div>
                  <div className="font-medium text-primary">{viewingItem.archivedBy}</div>
                </div>
                <div>
              ...
              <div>
                ...
                <div className="text-primary bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                  {viewingItem.reason}
                </div>
              </div>
              ...
              {!viewingItem.canRestore && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-medium text-red-800 mb-1">Cannot Restore</h4>
            ...
            <div className="border-t border-border px-6 py-4 flex justify-end gap-3">
              <button
                ...
                className="px-6 py-2.5 bg-primary border border-border rounded-lg text-secondary hover:bg-gray-50 transition-colors"
              >
                Close
              </button>
              {viewingItem.canRestore && (
